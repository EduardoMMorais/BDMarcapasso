...
```{r}
path <- "../EDA/auxiliar/model_selection/performance"
...
df %>%
  filter(Target == 'Readmission') %>%
  select(Period, Model, AUC, `Lower Limit`, `Upper Limit`) %>%
  niceFormatting(caption = 'Readmission models comparison', label = 3)

df %>%
  filter(Target == 'Death') %>%
  select(Period, Model, AUC, `Lower Limit`, `Upper Limit`) %>%
  niceFormatting(caption = 'Death models comparison', label = 4)
```
