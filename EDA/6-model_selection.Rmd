...
if (balancing_method == 'oversampling'){
  df <- upSample(x = df_train %>% select(-all_of(outcome_column)),
                 y = df_train[[outcome_column]],
                 yname = outcome_column)
  df %>% group_by(!!sym(outcome_column)) %>% tally
} else if (balancing_method == 'undersampling'){
  df <- downSample(x = df_train %>% select(-all_of(outcome_column)),
                   y = df_train[[outcome_column]],
                   yname = outcome_column)
  df %>% group_by(!!sym(outcome_column)) %>% tally 
}
