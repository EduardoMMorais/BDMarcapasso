...
max_auc_loss <- 0.005 # Max accepted loss of AUC for reducing num of features
```
...
selection_results %>%
  ...
  geom_line() + 
  geom_ribbon(alpha = .3) # Add auc threshold
...
lightgbm_grid <- grid_latin_hypercube(
  finalize(mtry(),
           df_train %>% select(all_of(c(selected_features, outcome_column)))),
  dials::trees(range = c(100L, 300L)),
  min_n(),
  tree_depth(),
  learn_rate(),
